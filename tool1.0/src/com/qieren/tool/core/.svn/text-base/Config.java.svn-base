...
public class Config {
    ...
    public class Project {
        ...
        private List<String> menuCodes = new ArrayList<String>();
        private List<String> buttons = new ArrayList<String>();
        private List<String> itemNodeCodes = new ArrayList<String>();
		...
		public void addMenuCodes(String menuCode) {
		...
		public List<String> getItemNodeCodes() {
			return itemNodeCodes;
		}

		public void addItemNodeCodes(String itemNodeCode) {
			itemNodeCodes.add(itemNodeCode);
		}
    ...
    private void configProject(Element element) throws Exception{
        ...
        configProjectElementByExcel(project);
    ...
    private void configProjectElementByExcel(Project project) throws Exception {
    	InputStream fs= Thread.currentThread().getContextClassLoader().getResourceAsStream("业务流程事项环节编码信息表.xlsx");  //获取Excel路径 
        XSSFWorkbook wb = new XSSFWorkbook(fs);
        XSSFSheet sheet=wb.getSheetAt(0);  //获取到工作表，因为一个excel可能有多个工作表  
        XSSFRow proRow=sheet.getRow(1);  //获取第一行
        project.setItemCode(getCellFormatValue(proRow.getCell(1)));
        int rowcollum = sheet.getLastRowNum();
        for(int i=1;i<=rowcollum;i++)
        {
        	proRow=sheet.getRow(i);
        	project.addButtons(getCellFormatValue(proRow.getCell(6)).trim()+","+getCellFormatValue(proRow.getCell(7)).trim());
        	project.addMenuCodes(getCellFormatValue(proRow.getCell(3)).trim()+","+getCellFormatValue(proRow.getCell(4)).trim());
        	project.addItemNodeCodes(getCellFormatValue(proRow.getCell(5)).trim());
        }
    }
    /**
     * 根据XSSFCell类型设置数据
